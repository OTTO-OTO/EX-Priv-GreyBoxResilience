...
def main(**kwargs):
    ...
    full_priv_score = ResilienceCal(full_priv, train_config, train_dataloader, rank)
    ...
    for wipe_layers in range(1,llama_config.num_hidden_layers):
        ...
        temp_score = ResilienceCal(model, train_config, train_dataloader, rank)
        end_time = time.time()
...
def ResilienceCal(model, train_config, eval_dataloader, local_rank):
