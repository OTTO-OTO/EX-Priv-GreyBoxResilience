...
from training_utils.policies import AnyPrecisionAdamW, apply_fsdp_checkpointing, apply_fsdp_checkpointing_falcon
...
def main(**kwargs):
    ...
    if local_rank == 0 and not os.path.exists(f'./project-falcon/loss_txtfiles/{train_config.loss_store_path}/{train_config.group_name}'):
    ...
    if train_config.enable_fsdp and train_config.low_cpu_fsdp:
        ...
        if rank == 0:
            falcon_config = FalconConfig.from_pretrained(train_config.model_path)
            falcon_config.use_cache = use_cache
            ...
            # 得到的仅仅是config
            falcon_config = FalconForCausalLM.from_pretrained(train_config.model_path)
            falcon_config.use_cache = use_cache
    ...
    if train_config.model_name == "falcon":
        falcon_config = FalconConfig.from_pretrained(train_config.model_path)
        falcon_config.use_cache = use_cache
        layer_container = FalconForCausalLM(falcon_config)
    ...
    for k in range(train_config.target_layer,train_config.target_layer+train_config.wipe_layer):
        state_dict[f'transformer.h.{k}.self_attention.query_key_value.weight'].data.copy_(new_state_dict[f'transformer.h.{k}.self_attention.query_key_value.weight'].data)
        state_dict[f'transformer.h.{k}.self_attention.dense.weight'].data.copy_(new_state_dict[f'transformer.h.{k}.self_attention.dense.weight'].data)
        state_dict[f'transformer.h.{k}.mlp.dense_h_to_4h.weight'].data.copy_(new_state_dict[f'transformer.h.{k}.mlp.dense_h_to_4h.weight'].data)
        state_dict[f'transformer.h.{k}.mlp.dense_4h_to_h.weight'].data.copy_(new_state_dict[f'transformer.h.{k}.mlp.dense_4h_to_h.weight'].data) # (linear layer)
        state_dict[f'transformer.h.{k}.input_layernorm.weight'].data.copy_(new_state_dict[f'transformer.h.{k}.input_layernorm.weight'].data)
        state_dict[f'transformer.h.{k}.input_layernorm.bias'].data.copy_(new_state_dict[f'transformer.h.{k}.input_layernorm.bias'].data)  # (feed forward)
    ...
    if train_config.enable_fsdp:
        ...
        if fsdp_config.fsdp_activation_checkpointing:
            apply_fsdp_checkpointing_falcon(model)
